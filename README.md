...
Technically, panns is only a small function module in one of our ongoing projects - [Kvasir Project](https://www.cs.helsinki.fi/liang.wang/kvasir). Kvasir project aims at exploring innovative ways of seamlessly integrating intelligent content provision and recommendation in the web-browsing. For the details of Kvasir project, please visit our project website. The reason we release it as a separate package is we realized it is actually very difficult to find an easy-to-use tool which can perform efficient k-NN search with satisfying accuracy in high dimensional space. High dimensionality in this context refers to those datasets having **hundreds of features**, which is already far beyond the capability of standard [k-d tree](http://en.wikipedia.org/wiki/K-d_tree).
